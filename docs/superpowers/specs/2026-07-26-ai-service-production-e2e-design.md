...
4. The E2E runner waits at most 180 seconds for container health and uses `docker compose exec ai-service` for the unexposed HTTP boundary.
...
Deployment uses a reviewed, pinned Git commit. The runbook acceptance path is: `git fetch`, `git checkout <commit>`, create `.env.production` and `.env.production.secrets` with mode `0600`, validate root Compose, build/start dependencies and `ai-service`, wait for health, run smoke and benchmark, retrieve the JSON report, and inspect bounded container logs. Mongo index creation remains the existing idempotent lifespan operation; no migration/reset command is added. Rollback selects the previously recorded commit, checks it out, rebuilds/recreates only `ai-service`, retains MongoDB/Redis volumes, then verifies health and reads the pre-rollback E2E session. No destructive database reset is part of E2E or rollback.
