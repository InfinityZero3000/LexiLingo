...
from app.crud.games import create_game_session, get_game_session_for_update
        ...
        expected_items=[
            {
                ...
                "xp_value": next(
                    w.xp_value for w in selected if str(w.id) == pair["word_id"]
                ),
            }
            for pair in pairs
        ],
...
async def get_hangman_word(
    ...
    if words:
        ...
        session = await create_game_session(
            ...
            session_metadata={
                "hint_costs": [hint2_cost, hint3_cost],
                "max_lives": 6,
            },
        )
        return {
...
async def complete_game_session(
    ...
    if session.user_id != current_user.id:
        raise HTTPException(
            ...
            detail="This game session belongs to another user.",
        )
    if session.completed_at is not None and session.xp_awarded:
        existing_award = await get_existing_xp_award(
            db=db,
            user=current_user,
            source="game",
        ...
        if existing_award is None:
            raise HTTPException(
                ...
                detail="Completed game session has no XP transaction.",
            )
        ...
        return GameSessionCompleteResponse(
            ...
            base_xp=score_result.get(
                "base_xp",
                existing_award.base_xp,
            ),
    ...
    if session.completed_at is not None or session.xp_awarded:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Game session is in an inconsistent completion state.",
        )
    ...
    if duration_seconds < 5:
        raise HTTPException(
            ...
            detail="Game session completed too quickly.",
        )
    ...
    try:
        ...
        if unlocked_achievements:
            ...
            await delete_cached(
                build_cache_key("wallet", user_id=str(current_user.id))
            )
    except Exception:
        logger.exception(
            "Achievement evaluation failed after game session %s",
            session.id,
        )
        await db.rollback()
    ...
    return GameSessionCompleteResponse(
        ...
        award_status=(
            "awarded" if xp_result.xp_awarded > 0 else "no_reward"
        ),
