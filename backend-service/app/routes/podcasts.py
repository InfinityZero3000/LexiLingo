...
async def proxy_image(url: str = Query(..., description="The image URL to proxy")):
    ...
    # _stream_public_url is IP-pinned and revalidated on every redirect hop
    # (see app/core/safe_http.py) — a plain follow_redirects=True here would
    # let a redirect chain reach a private address after the URL itself
    # passed inspection.
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            resp = await _stream_public_url(
                client,
                url,
...
async def generate_transcript(
    ...
    cache_key = f"podcasts:transcript:{transcript_fingerprint}"
    cache_service = APICacheService(db)
    ...
    async def _fetch_and_transcribe():
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await _stream_public_url(client, audio_url)
                try:
...
async def _parse_with_feedparser(feed_url: str, limit: int) -> dict:
    ...
    feedparser.parse(url) does its own fetching (urllib) if handed a URL
    string, bypassing the SSRF-safe pinned fetch entirely — including
    following redirects with no revalidation. Fetch the bytes ourselves via
    safe_get and hand feedparser the content instead of the URL.
    """
    import feedparser  # type: ignore

    async with httpx.AsyncClient(timeout=20.0) as client:
        response = await _safe_get(
            client, feed_url, headers={"User-Agent": "LexiLingo/1.0 RSS Reader"}
        )
        response.raise_for_status()
