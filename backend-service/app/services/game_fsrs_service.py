...
def _apply_lapse(item, *, now: datetime) -> tuple[float, int, int]:
    ...
    try:
        async with db.begin_nested():
            ...
            result = await db.execute(
                ...
                .where(
                    func.lower(VocabularyItem.word) == game_word.word.lower()
                )
                .limit(1)
            )
        ...
        async with db.begin_nested():
            ...
            result = await db.execute(
                select(UserGrammarItem).where(
                    ...
                    UserGrammarItem.grammar_item_id == grammar_item.id,
                )
            )
