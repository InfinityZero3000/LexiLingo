...
from api.services.graph_cag.nodes_v2 import retrieve_node
...
async def test_dynamic_intent():
    ...
    state1 = GraphCAGState(user_input="Hello, how are you?")
    ...
    state2 = GraphCAGState(user_input="Bạn có biết tin tức về ông Elon Musk mới đây không?")
    await retrieve_node(state2)

    print("\n--- TEST 3: Specific Tech (Clawbot) ---")
    state3 = GraphCAGState(user_input="Do you know the latest technology called Clawbot?")
