...
Future<void> _pumpWrapped(WidgetTester tester, Widget child) async {
...
void main() {
  ...
  group('GameLoadState', () {
    testWidgets('shows error_outline icon', (tester) async {
      await _pumpWrapped(
        tester,
        GameLoadState(message: 'Network error occurred.', onRetry: () async {}),
      );
    ...
    testWidgets('shows a retry button with refresh icon', (tester) async {
      await _pumpWrapped(
        tester,
        GameLoadState(message: 'Failed to load.', onRetry: () async {}),
      );
    ...
    testWidgets('retry callback is invoked on button tap', (tester) async {
      ...
      await _pumpWrapped(
        tester,
        GameLoadState(message: 'Error.', onRetry: () async => retryCount++),
      );
    ...
    testWidgets('renders with network error message', (tester) async {
      ...
      await _pumpWrapped(
        tester,
        GameLoadState(message: networkMessage, onRetry: () async {}),
      );
    ...
    testWidgets('renders with empty payload message', (tester) async {
      ...
      await _pumpWrapped(
        tester,
        GameLoadState(message: emptyMessage, onRetry: () async {}),
      );
    ...
    testWidgets('renders with malformed payload message', (tester) async {
      ...
      await _pumpWrapped(
        tester,
        GameLoadState(message: malformedMessage, onRetry: () async {}),
      );
    ...
    testWidgets('layout is constrained to maxWidth 360', (tester) async {
      ...
      final maxWidthBox = constrainedBoxes.firstWhere(
        ...
        orElse: () =>
            throw TestFailure('No ConstrainedBox with maxWidth 360 found'),
      );
    ...
    testWidgets('stale error from one game does not leak to next game state', (
      ...
      await _pumpWrapped(
        tester,
        GameLoadState(message: errorMsgA, onRetry: () async {}),
      );
      ...
      await _pumpWrapped(
        tester,
        GameLoadState(message: errorMsgB, onRetry: () async {}),
      );
  ...
  group('GameLoadState — loading state (Scaffold + indicator)', () {
    ...
    testWidgets('GameLoadState replaces loading indicator on error', (
      tester,
    ) async {
      // Loading state
      await _pumpWrapped(
